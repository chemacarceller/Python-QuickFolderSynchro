...
def signal_handler(sig, frame, isRecursiveExecution, sourceDirectory):
    ...
    logger.LOG_INFO(f"Source Directory : {sourceDirectory} --> Signal {signal.Signals(sig).name} ({sig}) received : Starting the children's cleanup on the parent process and saving logs and exiting on each process...")
...
try :
    ...
    # Se obtiene el Singleton, el proceso padre lo crearia
    logger = LogFileWriter.Writer.get_instance()

    # Initializing the variables that will hold the source and target directories
    ...
    if not isRecursiveExecution :
        ...
        # El padre es el ÚNICO que arranca el hilo de escritura a disco
        logger.start_worker()
        ...
        # Also in a recursive execution, the source and target directories are extracted from the command line arguments
        sourceDirectory = sys.argv[1]
        targetDirectory = sys.argv[2]
    ...
    logger.LOG_INFO(f"SEARCHING FOR FILES IN {sourceDirectory} : ", False)
    ...
    if len(os.listdir(sourceDirectory)) == 0 :
        ...
        logger.LOG_INFO(f"STATISTICS FOR {sourceDirectory} : ", False)
        ...
        logger.LOG_INFO(f"SEARCHING FOR FILES IN {targetDirectory} : ", False)
        ...
        # These statistics are printed only in the log file.
        logger.LOG_INFO(f"STATISTICS FOR {targetDirectory} : " , False)
    ...
    for fileItem in sorted(os.listdir(sourceDirectory)) :
        ...
        try :
            ...
            if os.path.isdir(sourcePath) :
                ...
                logger.LOG_INFO(f"The directory {sourcePath} is going to be processed",False)
